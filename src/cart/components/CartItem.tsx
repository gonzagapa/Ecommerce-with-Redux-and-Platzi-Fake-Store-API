...
export function CartItem({imageURL,price,title,id,amount}:Props) {
  ...
  return (
    <article data-id={id} 
    className="dark:border-baby border-highlight border-2 rounded-md p-2.5 max-w-lg flex items-center gap-4 w-full relative shadow-md shadow-gray-500">
        ...
        <section className="flex flex-col gap-5 justify-start">
            <h3 className="text-base md:text-2xl line-clamp-2">{title}</h3>
            <div className="flex flex-col justify-between gap-1">
                <p className="md:text-xl italic text-gray-400">price:{currency.format(price)}</p>
